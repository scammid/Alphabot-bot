...
async function enterRaffle(apiKey, slug, mintAddress, retries = 2) {
  try {
    const client = createClient(apiKey);
    const body = { slug };
    if (mintAddress) body.mintAddress = mintAddress;
...
module.exports = { validateApiKey, getOpenRaffles, getMyCommunityRaffles, enterRaffle, checkWins };
