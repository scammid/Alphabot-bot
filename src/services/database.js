...
async function getDb() {
  ...
  const migrations = [
    ...
    "CREATE TABLE IF NOT EXISTS mint_reminders (id INTEGER PRIMARY KEY AUTOINCREMENT, discord_id TEXT, raffle_slug TEXT, raffle_name TEXT, mint_date INTEGER, wallet_used TEXT, reminded_72h INTEGER DEFAULT 0, reminded_24h INTEGER DEFAULT 0, reminded_1h INTEGER DEFAULT 0, reminded_start INTEGER DEFAULT 0, created_at TEXT DEFAULT (datetime('now')))",
  ];
...
async function addMintReminder(discordId, raffleSlug, raffleName, mintDate, walletUsed) {
  ...
  if (!exists) {
    run('INSERT INTO mint_reminders (discord_id, raffle_slug, raffle_name, mint_date, wallet_used) VALUES (?,?,?,?,?)',
...
async function getPendingReminders() {
  await getDb();
  return all('SELECT * FROM mint_reminders WHERE mint_date > 0');
}

async function markReminder(id, type) {
  await getDb();
  run(`UPDATE mint_reminders SET ${type}=1 WHERE id=?`, [id]);
}
